...
html=html.replace('</head>', '<style>body:not(.signed-in)>aside,body:not(.signed-in)>main{display:none}#login-card{max-width:440px;margin:10vh auto;padding:42px;background:white;border:1px solid #e8e4f3;border-radius:20px;box-shadow:0 20px 70px #39305e12}#login-card button{margin-top:12px}#auth-msg{color:#a63232;line-height:1.5}#sign-out{position:fixed;bottom:12px;left:20px;z-index:3}body.signed-in #login-card{display:none}body:not(.signed-in) #sign-out{display:none}</style></head>')
html=html.replace('</body>', '''<section id="login-card"><div class="eyebrow">A LITTLE CLARITY. A LOT MORE CONTROL.</div><h1>Campus Wallet<span>.</span></h1><p class="muted">Sign in to keep your expenses private and saved across devices.</p><form id="login-form"><label>Email<input type="email" name="email" required autocomplete="email"></label><label>Password<input type="password" name="password" required minlength="8" autocomplete="current-password"></label><button class="primary full" type="submit">Sign in</button><button class="full" id="sign-up" type="button">Create account</button><p id="auth-msg" role="status"></p></form></section><button id="sign-out">Sign out</button></body>''')
